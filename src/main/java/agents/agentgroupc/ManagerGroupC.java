...
public class ManagerGroupC extends OSPABA.Manager {
	...
	public void processRequestResponseMoveWorker(MessageForm message) {
        ...
        if (product.getProductActivity() == ProductActivity.CUT)
            this.startProcess(message, product, Id.processVerifyParts);
        else {
    ...
    private void tryStartWork() {
        ...
        } else if (product.getProductActivity() == ProductActivity.VERIFIED) {
            this.startProcess(messageProduct, product, Id.processMorenie);
        } else {
            this.startProcess(messageProduct, product, Id.processVerifyParts);
        }
    }
...
//			this.tryStartWorkOnOrder();
	...
	//meta! sender="ProcessVerifyParts", id="125", type="Finish"
	public void processFinishProcessVerifyParts(MessageForm message)
	{
        MyMessage msg = (MyMessage) message;
        this.startProcess(msg, msg.getProduct(), Id.processMorenie);
	}
	...
	public void processMessage(MessageForm message)
	{
		...
		case Mc.finish:
			...
			case Id.processVerifyParts:
				processFinishProcessVerifyParts(message);
			break;
